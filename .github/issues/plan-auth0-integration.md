...
Add Auth0 authentication to the BlazorApp.Web project. The Auth0 domain and clientId values are already configured in
the AppHost user secrets and must be read from configuration, not hardcoded or stored in appsettings.json.
...
2. **Read Auth0 Settings from AppHost User Secrets**
  - Ensure BlazorApp.Web is configured to read the Auth0:Domain and Auth0:ClientId from the AppHost user secrets via
