...
namespace Web.Components.Features.Categories
{
	...
	public class CategoryTestFixture : IAsyncDisposable
	{
		...
		public MyBlogContext BlogContext { get; }
		...
		private class TestMyBlogContextFactory : Web.Data.IMyBlogContextFactory
		{
			...
			public Task<Web.Data.IMyBlogContext> CreateAsync(CancellationToken cancellationToken = default)
			{
		...
		public void SetupFindAsync(IEnumerable<Category> categories)
		{
			...
			CategoriesCollection
				.FindAsync<Category>(Arg.Any<MongoDB.Driver.FilterDefinition<Category>>(), Arg.Any<MongoDB.Driver.FindOptions<Category, Category>>(), Arg.Any<CancellationToken>())
		...
		/// </summary>
		public Handler CreateGetHandler() => new Handler(BlogContext, Logger);
		...
		public void ApplyTo(BunitContext ctx)
		{
			...
			ctx.Services.AddScoped<Web.Components.Features.Categories.CategoryDetails.GetCategory.IGetCategoryHandler>(_ => getHandler);
			...
			ctx.Services.AddScoped<Web.Components.Features.Categories.CategoryEdit.EditCategory.IEditCategoryHandler>(_ => editHandler);
		...
		/// Tests can register this into a bUnit TestContext or the test DI container.
		/// </summary>
		public EditCategory.Handler CreateEditHandler()
		{
