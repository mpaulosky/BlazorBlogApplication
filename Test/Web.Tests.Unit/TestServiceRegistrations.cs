...
// JSInterop not required for these tests by default; keep imports minimal

// Lightweight test helpers for registering common services into a BunitContext
public static class TestServiceRegistrations
{
	// Register a minimal Auth0Service replacement (no network calls)
	public static void RegisterAuth0Service(BunitContext ctx)
	{
	...
	public static MyBlogContext RegisterMyBlogContext(BunitContext ctx)
	{
	...
	public static void RegisterHandlerSubstitutes(BunitContext ctx)
	{
		...
		if (!isEitherRegistered(typeof(Web.Components.Features.Categories.CategoryList.GetCategories.IGetCategoriesHandler), typeof(Web.Components.Features.Categories.CategoryList.GetCategories.Handler)))
		{
			...
			ctx.Services.AddScoped<Web.Components.Features.Categories.CategoryList.GetCategories.IGetCategoriesHandler>(sp =>
			{
				try
				{
					var concrete = sp.GetService(typeof(Web.Components.Features.Categories.CategoryList.GetCategories.Handler));
					if (concrete is Web.Components.Features.Categories.CategoryList.GetCategories.IGetCategoriesHandler asInterface)
					{
						return asInterface;
					}
				}
		...
		if (!isEitherRegistered(typeof(Web.Components.Features.Categories.CategoryCreate.CreateCategory.ICreateCategoryHandler), typeof(Web.Components.Features.Categories.CategoryCreate.CreateCategory.Handler)))
		{
			...
			ctx.Services.AddScoped<Web.Components.Features.Categories.CategoryCreate.CreateCategory.ICreateCategoryHandler>(sp =>
			{
				try
				{
					var concrete = sp.GetService(typeof(Web.Components.Features.Categories.CategoryCreate.CreateCategory.Handler));
					if (concrete is Web.Components.Features.Categories.CategoryCreate.CreateCategory.ICreateCategoryHandler asInterface)
					{
						return asInterface;
					}
				}
	...
	public static void RegisterCategoryHandlers(BunitContext ctx)
	{
		...
		// call BuildServiceProvider() here because Bunit will lock the services
	...
	// Register NavigationManager and a logger if not present
	public static void RegisterCommonUtilities(BunitContext ctx)
	{
		if (!ctx.Services.Any(sd => sd.ServiceType == typeof(Microsoft.AspNetCore.Components.NavigationManager)))
		{
			ctx.Services.AddSingleton<Microsoft.AspNetCore.Components.NavigationManager, BunitNavigationManager>();
		}

		// Add a generic logger factory if none present
		if (!ctx.Services.Any(sd => sd.ServiceType == typeof(ILoggerFactory)))
		{
		...
		{
			// Swallow - registration of substitutes is best-effort for test safety.
		}
	}
	...
	public static void RegisterAll(BunitContext ctx)
	{
		...
		// explicitly in the test so behavior matches the package and version they use.

		// Intentionally do not build the service provider here. Tests should set up
		// navigation state after services are registered if they need to navigate.
	}
